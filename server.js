...
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || 'Welkom200!';
...
function adminAuth(req, res, next) {
  ...
  res.status(401).json({ error: 'Onbevoegd. Stuur X-Admin-Password header mee.' });
}
...
app.listen(PORT, () => {
  ...
  console.log(`✓ Admin wachtwoord: ${ADMIN_PASSWORD === 'Welkom200!' ? '(standaard)' : '(aangepast)'}`);
});
